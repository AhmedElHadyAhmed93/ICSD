...
export class BranchBankComponent implements OnInit, AfterViewInit {
  ...
  constructor(private serviceApi: BranchBankService,
  ...
  private applySave = function(item) {
    ...
    item.ParentID = this.selectedMainBanks[0].ID;
  ...
  OnMainBankDeSelect() {
    this.SelectedMainBankID = null;
    this.refreshDataSource([]);

  }
  MainBankSelected(event) {
    this.Cities = [];
    this.SelectedMainBankID = event.ID;
    this.serviceApi.GetAllBranchBanks(event.ID).subscribe(list => {
      this.refreshDataSource(list);
    });
  }
